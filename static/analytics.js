...
// HOTFIX 9.6.1 — contrato Research pesado actual: 60 celdas.
const RF96_RESEARCH_TARGET = 60;

function rf96NormTf(value) {
    const raw = String(value || '').trim().toUpperCase();
    if (raw === '30MIN' || raw === '30M') return '30M';
    if (raw === '1D' || raw === '1DAY') return '1D';
    if (raw === '1W' || raw === '1WEEK') return '1W';
    return raw;
}

function rf96CandidateInActiveContract(row) {
    row = row || {};
    const symbol = String(row.symbol || row.scope?.symbol || '').toUpperCase().replace('/', '-');
    const tf = rf96NormTf(row.timeframe || row.scope?.timeframe);
    const family = String(row.market_family || row.scope?.market_family || '').toUpperCase();
    const isFutures = family === 'CRYPTO_FUTURES';
    if (isFutures) {
        const allowed = {
            'BTC-USDT': new Set(['30M','1H','2H','4H','12H','1D']),
            'XRP-USDT': new Set(['30M','1H','2H','4H','12H']),
            'LINK-USDT': new Set(['30M','1H','2H','4H']),
            'SUI-USDT': new Set(['30M','1H','2H']),
        };
        return Boolean(allowed[symbol] && allowed[symbol].has(tf));
    }
    const spotFamilies = new Set(['CRYPTO_SPOT','PAXG_BTC','PAXG_USDT','SPOT']);
    return spotFamilies.has(family)
        && new Set(['BTC-USDT','PAXG-USDT','PAXG-BTC']).has(symbol)
...
function renderV1EntryAudit(data) {
    ...
    const spot = execution.spot || {};

    const renderMarket = (prefix, row, stricter=false) => {
        const n = Number(row?.n_with_forensics || 0);
        q5SetText(`${prefix}-n`, n ? n.toLocaleString() : 'SIN MUESTRA');
        q5SetText(`${prefix}-reach`, n ? loPct(row?.entry_reach_rate_pct) : '--');
        q5SetText(`${prefix}-direct-stop`, n ? loPct(row?.direct_stop_rate_pct) : '--');
        q5SetText(`${prefix}-mfe`, n ? loR(row?.avg_mfe_r) : '--');
        q5SetText(`${prefix}-mae`, n ? loR(row?.avg_mae_r) : '--');
    ...
    renderMarket('v1-entry-fut', futures, true);
    renderMarket('v1-entry-spot', spot, false);
}

function renderLearningObservatory(data) {
    window.__v1LastAnalytics = data || {};
    updateV1ReviewTraderNote();
    renderV1EntryAudit(data || {});
    const observatory = data?.learning_observatory_v1 || {};
...
async function loadLearningGovernanceStatus() {
    ...
    results.forEach(item => {
        ...
        if (kind === 'autopilot' && json.success) {
            ...
            const futuresResolved = Number((evidence.total || {}).resolved || 0);
            window.__v1LastGovernance = {
                positiveAuthority,
                futuresResolved,
                state,
                strategyVeto,
                riskGrowthAllowed: Boolean(governance.risk_growth_allowed)
            };
            updateV1ReviewTraderNote();
...
async function loadResearchFederationAnalytics(){
    ...
    try{
        ...
        window.__v1LastResearchSummary = data || {};
        updateV1ReviewTraderNote();
        ...
        const strategyCardHtml=row=>{
            const card=row?.strategy_card||{};
            const raw=(card.raw_spec&&typeof card.raw_spec==='object')?card.raw_spec:(row?.strategy_spec||{});
            const hasRich=Boolean(card.display_name||card.signal_logic||card.technical_id);
            const name=card.display_name||uiHumanLabel(row?.strategy_family||'Especialista causal');
            const technicalId=card.technical_id||row?.strategy_id||row?.candidate_key||'--';
        ...
        if(liveBody){
            liveBody.innerHTML=validatedRows.length?validatedRows.map(row=>{
                ...
                const guardianReplay=row.guardian_operational_replay||{};
                const guardianDelta=guardianReplay.delta_expectancy_r_vs_original;
        ...
        if(covBody){
            const groups=[
                ...
                ['Spot 4h · COMPRA/VENTA','4H',6,'SPOT'],
                ['Spot 12h · COMPRA/VENTA','12H',6,'SPOT'],
                ['Spot 1D · COMPRA/VENTA','1D',6,'SPOT'],
